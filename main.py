...
async def start(message: types.Message):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			await message.answer('Добро пожаловать, Долбаёб.', reply_markup=kb.menu)
		else:
			await message.answer('Приветствую Пиздюг, это бот-саппорт.\nНапиши мне свой вопрос и я отправлю его администрации.\nЗа спам/флуд - ЧС!')
	else:
...
async def cal(call, state: FSMContext):
	if 'ans' in call.data:
		...
		ids = call.data[:a]
		await call.message.answer('Введите ответ Долбаёбу:', reply_markup=kb.back)
